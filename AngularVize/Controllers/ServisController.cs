...
namespace AngularVize.Controllers
{
    public class ServisController : ApiController
    {
        ...
        {
            TDersler kayit = db.TDersler.Where(s => s.dersId == model.dersId).FirstOrDefault();
        ...
        {
            if (db.TKayit.Count(s => s.kayitDersId == model.kayitDersId && s.kayitOgrId == model.kayitOgrId && s.kayitOgrtId == model.kayitOgrtId) > 0)
            {
                sonuc.islem = false;
        ...
        [HttpGet]
        [Route("api/dosyadersliste/{ogrId}")]
